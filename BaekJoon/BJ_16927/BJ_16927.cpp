...
int main() {
	...
	while (nowWidthSize > 0 && nowHeightSize > 0) {
		if (2 * nowHeightSize + 2 * nowWidthSize - 4 < 0)
			nowR = (R % 4);
		else
			nowR = R % (2 * nowHeightSize + 2 * nowWidthSize - 4);
		for(int rCount = 0; rCount < nowR; rCount++){
			temp = matrix[startPoint.first][startPoint.second]; // ¿Ş, À§
			int i, j;
			// ¿Ş <- ¿À
			// i, j : Ã¹ µµÂøÁö
			i = startPoint.first;
